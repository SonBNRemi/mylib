package com.AlarmClock.LoudAlarm.ChallengesAlarmClock.ads
...
//        if (!isGDPR()) {
//            println("sonbn callback !isGDPR: ${consentInformation.isConsentFormAvailable}")
...
//        }
        ...
        consentInformation.requestConsentInfoUpdate(
            ...
            params,
            {
                ...
                if (!isGDPR()){
                    println("sonbn callback !isGDPR")
                    mCallback?.initializeMobileAdsSdk(true)
                    return@requestConsentInfoUpdate
                }
                UserMessagingPlatform.loadAndShowConsentFormIfRequired(activity) { consentForm ->
                    printLogs("loadConsentForm: ${consentForm?.toString()}")
                    println("sonbn callback loadConsentForm")
            ...
            },
            {
                ...
                println("sonbn callback requestConsentInfoUpdate error: ${it.message}")
                printLogs("requestConsentInfoUpdate error")
                if (!isGDPR()){
                    println("sonbn callback !isGDPR requestConsentInfoUpdate error")
                    mCallback?.initializeMobileAdsSdk(true)
                    return@requestConsentInfoUpdate
                }
                mCallback?.initializeMobileAdsSdk(canShowAds() || canShowPersonalizedAds())
